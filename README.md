...
`pyttsx3`: (본 프로젝트 노트북에 적용) 오프라인 환경에서 동작하며 Mac, Windows, Linux 크로스 플랫폼을 지원. (로컬 연산)
...
학습 내용: 현재 터미널 백그라운드 환경에서 동작하는 STT/TTS 시스템을 한 단계 발전시켜, 파이썬 기반 웹 프레임워크인 `Streamlit`을 활용해 사용자 친화적인 웹 UI를 입히는 방법론을 참고.
